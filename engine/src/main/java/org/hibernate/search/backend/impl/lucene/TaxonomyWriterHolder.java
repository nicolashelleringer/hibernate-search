...
class TaxonomyWriterHolder {
	 ...
	 */
	private final AtomicReference<TaxonomyWriter> writer = new AtomicReference<TaxonomyWriter>();
	...
	TaxonomyWriterHolder(ErrorHandler errorHandler, DirectoryBasedIndexManager indexManager) {
		...
		this.directoryProvider = indexManager.getTaxoDirectoryProvider();
	...
	public TaxonomyWriter getTaxonomyWriter(ErrorContextBuilder errorContextBuilder) {
		TaxonomyWriter taxonomyWriter = writer.get();
		if ( taxonomyWriter == null ) {
			writerInitializationLock.lock();
			try {
				taxonomyWriter = writer.get();
				if ( taxonomyWriter == null ) {
					try {
						taxonomyWriter = createNewTaxonomyWriter();
						log.trace( "IndexWriter opened" );
						writer.set( taxonomyWriter );
					}
	 ...
	 */
	private TaxonomyWriter createNewTaxonomyWriter() throws IOException {
		...
		return new DirectoryTaxonomyWriter( directoryProvider.getDirectory(), writerConfig.getOpenMode() ) {
		};
	}
	 ...
	 */
	public void commitTaxonomyWriter(ErrorContextBuilder errorContextBuilder) {
		TaxonomyWriter taxonomyWriter = writer.get();
		if ( taxonomyWriter != null ) {
			try {
	...
	/**
	 * @see #commitTaxonomyWriter(ErrorContextBuilder)
	 */
	public void commitTaxonomyWriter() {
		commitTaxonomyWriter( null );
	}
	...
	public void closeTaxonomyWriter() {
		final TaxonomyWriter toClose = writer.getAndSet( null );
		if ( toClose != null ) {
			try {
				toClose.close();
	...
	public void forceLockRelease() {
		...
		try {
			try {
				TaxonomyWriter intaxonomyWriter = writer.getAndSet( null );
				if ( intaxonomyWriter != null ) {
					intaxonomyWriter.close();
					log.trace( "IndexWriter closed" );
				}
			}
			finally {
	 ...
	 */
	public TaxonomyReader openDirectoryTaxonomyReader() {
		try {
			return new DirectoryTaxonomyReader( directoryProvider.getDirectory() );
		}
